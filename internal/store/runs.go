...
const runCols = `id, task_id, project_id, COALESCE(bot_member_id::text, ''), status, detail, created_at, updated_at, started_at, finished_at`

func scanRun(row interface{ Scan(...any) error }, r *models.Run) error {
	return row.Scan(&r.ID, &r.TaskID, &r.ProjectID, &r.BotMemberID, &r.Status, &r.Detail, &r.CreatedAt, &r.UpdatedAt, &r.StartedAt, &r.FinishedAt)
}

func (s *Store) InsertRun(ctx context.Context, r models.Run) error {
	_, err := s.q.Exec(ctx, `INSERT INTO runs (id, task_id, project_id, bot_member_id, status, detail, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $7)`,
		r.ID, r.TaskID, r.ProjectID, nullID(r.BotMemberID), r.Status, r.Detail, r.CreatedAt)
	return mapErr(err)
}
...
// UpdateRun writes status, detail and the lifecycle timestamps.
func (s *Store) UpdateRun(ctx context.Context, r models.Run) error {
	return one(s.q.Exec(ctx, `UPDATE runs SET status=$2, detail=$3, updated_at=$4, started_at=$5, finished_at=$6 WHERE id=$1`,
		r.ID, r.Status, r.Detail, r.UpdatedAt, r.StartedAt, r.FinishedAt))
}
