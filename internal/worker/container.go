...
func (c *Container) exec(worker string, commands map[string][]string) Exec {
	return func(ctx context.Context, job Job, emit acp.Handler) (string, error) {
		if job.Agent == "fake" {
			return acp.Run(ctx, acp.Config{Agent: "fake", WorkDir: job.Dir}, job.Prompt, emit)
		}
		...
		return acp.Run(ctx, acp.Config{Agent: job.Agent, Command: cmd, WorkDir: job.Dir}, job.Prompt, emit)
	}
}
