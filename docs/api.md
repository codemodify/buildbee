...
| `X-BuildBee-Agent: <name>` with `X-BuildBee-Bot: <bot id>` | a Bot's agent | that Bot's agent process; its Run changes are attributed to the Bot |
...
| `GET/PATCH /v1/projects/{id}` | Project with Members and Channels / `{name, auto_run, merge_policy: auto\|approval, max_runs, instructions, repo_url, default_branch, agent_image, archived}`; `auto_run` is [autopilot](autopilot.md); `agent_image` is the container image its AIs run in (`""` = the agent's own); `agent_permissions` is `auto` (agents act freely in their Run) or `ask` (each permission request is a Decision) |
| `POST /v1/runs/{id}/permission` | the Run's agent only: `{title, options: [{id, name, kind}]}` → a Decision (`action: permission`, `run_id`) on the Task, recommending the allow-once option. The agent polls `GET /v1/decisions/{id}` and gives its AI the chosen option; a Run that ends closes its open ones |
...
| `GET /v1/presence` | people with the app open, and `bots`: the Bots whose agent has been seen in the last 90 s, each with its `ai`, `host`, `process`, `slots` and `running`. `work` counts Runs per Bot ID (`{running, queued}`; the `""` key is work nobody is assigned, such as a merge), with `slots` and `running` totals |
...
| `GET/PATCH /v1/runs/{id}` | Run / `{status, detail}`: `pending → running → succeeded\|failed\|canceled`; people may only cancel; the Run's agent also reports `{summary, branch, pr_url}` |
...
| `GET /v1/runs/{id}/files/{file}` | the Run's agent only: an attachment of the Run's Task |
| `POST /v1/agent/claim` | a Bot's agent: `{ai, host, slots, wait_seconds ≤ 30}` → `200 {run, task, project, bot, handoffs, files}` or `204`. It returns that Bot's oldest queued Run, or one nobody is assigned |
| `POST /v1/agent/goodbye` | the agent is stopping; its Bot shows offline at once |
| `POST /v1/runs/{id}/heartbeat` | the Run's agent: renew the 60 s lease; returns the Run (status `canceled` means stop) |
...
Runs are a queue, routed by Bot. A Bot's agent claims its Runs, owns each until it finishes, and must heartbeat to keep it; only the claiming agent may append events, change a Run's status or attach Artifacts to it (`409` otherwise). A Run whose agent stops heartbeating is failed by the Server. See [agents.md](agents.md).
...
`GET /metrics` answers in Prometheus text format, without a login like the rest of a LAN Server: Runs by status, Runs running and queued per Bot (with its AI), Bots connected and their slots, people online, and HTTP requests by method and status with a duration histogram (long-polls and WebSockets are counted but not timed).
