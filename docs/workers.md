...
A Run's prompt carries the Project's guidance, the Bot's instructions, what the Run is for, the Task and the notes handed to the Bot. What people attached to the Task comes too: the claim lists the files, the worker downloads them beside the agent (in its home in a container or sandbox, in a scratch directory otherwise) and gives the agent their names and paths. Images also go into the prompt as pictures when the agent says it reads them (`promptCapabilities.image`), so a pasted screenshot is something the agent can look at, not just a path. At most 20 files and 64 MB per Run; anything larger is skipped and said so in the Run's log.
